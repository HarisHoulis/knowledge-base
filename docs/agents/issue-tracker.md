...
- **`gh api` flags**: `-f` sends strings, `-F` sends typed values (integers). GitHub rejects string values for integer properties (e.g. `sub_issue_id`, `issue_id`) with a 422 — use `-F` for any numeric field.
